...
class WordBook extends React.Component {
  ...
  convertImg() {
    if (this.props.memberData.image?.length < 5) {
      ...
      return (
        <img className="wordbookImg"
          src={
            this.props.memberData.image
          } alt="">
        </img>
      )
    }
...
class MyBook extends React.Component {
  ...
  renderShowBook() {
    ...
    if (this.props.showBook.length > 0) {
        ...
        <div className="myBookRender">
          ...
          <div className="newBookShow"
            ...
            <div className="addbookDecor">
              <div className="addBookTitle">{"✚" + "　" + "Create New Book"}
              </div>
            </div>
          </div>

          <div className="newBookHide"
            style={{
              display: this.props.addBookPop ? "block" : "none"
            }}>
            <form className="bookEnterFrame"
              onSubmit={(e) => { this.manageBookData(e) }} >
              <input className="bookEnter" type="text" placeholder="Enter a title, like: TOEIC words"
                onChange={(e) => this.getBookData(e)}>
              </input>
              <input className="bookSend" type="submit"
                value={
                  this.props.addBookSucceed ? "Succeed!" : "OK"
                }
                style={{
                  backgroundColor: this.props.addBookSucceed ? "black" : "#303545"
                }}
              >
              </input>
            </form>
            <div className="addbookShowPopX"
              onClick={() => {
            ...
            return (
              <div className="addBookShow bookformat" key={index}
                onClick={(event) => {
                  ...
                  window.location.href = ('https://kiwords-c058b.web.app/details/' + obj.bookID + "?" + obj.bookName)
                }}>
                <div className={"bookTitle " + obj.bookID} >
                  {obj.bookName}
                </div>
                <div className="bookBtnPlus"
                  onClick={(event) => {
                    event.stopPropagation(),
                    window.location.href = ('https://kiwords-c058b.web.app/addwords?' + obj.bookID) + "&" + obj.bookName
                  }}>Add Words
                </div>
              </div>
            )
      ...
      return (
        <div className="noBookInfor"
          style={{
            display: this.props.showBook.length >= 1 ? "none" : "flex"
          }}>
          <div className="noBookTitle">You have no Wordbook yet
          </div>
          <div className="noBookSubtitle">Create a new Wordbook to start adding flashcards.
          </div>
          <div className="noBookBtn"
            onClick={() => {
              this.props.popAddBook()
            }}>Create Wordbook
          </div>

          <div className="addbookpop"
            style={{
              display: this.props.addBookPop ? "block" : "none"
            }}>
            <div className="addbookpopX"
              onClick={() => {
                this.props.closeAddBook()
              }}>✕
            </div>
            <form className="addbookpopFrame"
              onSubmit={this.manageBookData}>
              <input className="addbookpopEnter" type="text" placeholder="Enter a title, like : 4000 Essential English Words "
                onChange={(e) => this.getBookData(e)}>
              </input>
              <input className="addbookpopSend" type="submit" value="Create">
              </input>
            </form>
          </div>
