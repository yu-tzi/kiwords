...
const QuizContainer = (props) => {
        ...
        .then((doc) => {
          ...
          if (doc.data().cards.length > 0) {
            for (let i = 0; i < doc.data().cards.length; i++) {
              ...
              let isSynonymsObjectBigEnough =
                typeof synonyms === "object" && synonyms.length > 2;
              let isAntonymsObjectBigEnough =
                typeof antonyms === "object" && antonyms.length > 2;
              let isSynonymsStringBigEnough =
                synonyms.toString().split(",").length > 2;
              let isAntonymsStringBigEnough =
                antonyms.toString().split(",").length > 2;
      ...
      if (count == props.showBook.length) {
        return (
          <div className="bookIDpopBox">
            ...
            <div className="bookIDpopSubtitle">
              Select WordBook As Quiz Resources
            </div>
            <select
              className="bookSelect"
              value={bookID}
              onChange={(e) => {
                setloadingAnimate(true), getBookID(e);
              }}
            >
              <option key={props.showBook.length + 1} value=" ">
                ———— Please choose one wordbook ————
              </option>
              {all}
            </select>
            ...
            <div
              className="bookPopContainer"
              style={{
                display: loadingAnimate ? "none" : "flex",
              }}
            >
              <div
                className="bookPopSend"
                onClick={() => {
                  if (bookName === "" || bookID === " ") {
                ...
                style={{
                  display:
                    validWords.length > 0 || !hasOption ? "block" : "none",
                }}
              >
                Next
              </div>
              <div
                className="bookPopCreateWording"
                style={{
              ...
              <div
                className="bookPopCreate"
                onClick={() => {
                  window.location.href =
                    "https://kiwords-c058b.web.app/wordbooks";
                }}
                style={{
                  display: hasOption ? "none" : "block",
                }}
              ...
              <div
                className="validWords"
                style={{
                  display: hasOption ? "block" : "none",
                }}
              >
                <div className="usableWords">
                  {"Useable wordcards in this book : " + validWords.length}
                </div>
                <img
                ...
                >
                  Only word cards with more then 2 synonyms and antonyms can be
                  used to generate quiz content.
                </div>
                <div
                  className="usableWordsWording"
                  style={{
  ...
  const renderTutorPop = () => {
    return (
      ...
      >
        ...
        >
          <div className="bookIDpop">{renderBookSelecter()}</div>
        </CSSTransition>
        ...
        >
          <div className="tutorPop">
            <div className="tutorPopBox">
              ...
              <img src="images/tutor.gif"></img>
              <div
                className="tutorPopBtn"
                onClick={() => {
                  setTutorPop(false);
                }}
              >
  ...
  return (
    <div className="quizContainer">
      <div
        className="bookIDpopBtn"
        onClick={() => {
          setBookIDpop(true);
        }}
      >
        {bookName === ""
          ? "Please Choose One of Your Wordbook"
          : "Resources :  " + bookName}
      </div>
      {renderTutorPop()}
      ...
      <QuizContext.Provider value={{ chooseItem, unChooseItem }}>
        ...
        >
          <div className="questionBlock">
            <div className="questionTitle">{"Question " + questionCount}</div>
          </div>
