    ...
    if (showBook.length > 0) {
      ...
      if (count == showBook.length) {
        return (
          ...
          >
            <select
              className="bookSelectOpt"
              onChange={(e) => {
                this.getBookID(e);
              }}
            >
              <option key={showBook.length + 1} value=" ">
                ———— Choose One Wordbook ————
              </option>
              {all}
            </select>
            <div
              className="selectBlockSend"
              onClick={() => {
    ...
    return (
      <div className="addWordBlock">
        <div className="addCardBox">
          ...
          <div className="cited">
            Dictionary Citing : <br></br>Merriam-Webster's Intermediate
            Thesaurus (1999). <br></br>Merriam-Webster Incorporated.
          </div>
        </div>
      </div>
    );
...
class WordBlock extends React.Component {
      ...
      <div key={i} className={"addWordBlocks" + " key" + i}>
        ...
        <div className="upper">
          <div className="word">
            <input
              type="text"
              placeholder="First ,let's enter any word !"
              className={i + " " + keyvalue}
              value={this.findValueViaKey(this.state.word, keyvalue) || ""}
              onChange={(e) => this.changeCardInput(e)}
            ></input>
            <div>WORD</div>
          </div>
  ...
  searchDic() {
    ...
    if (this.state.searchWord.replace(/\s+/g, "").length > 0) {
      let URL =
        "https://api.datamuse.com/sug?s=" + this.state.searchWord + "&max=5";
      fetch(URL)
        .then((res) => res.json())
        .then(
          (result) => {
  ...
  renderSearch(index) {
    let all = [];
    let isCurrentWordBox =
      this.state.menuPop && this.state.menuPopIndex === "key" + index;
  ...
  searchWordApi(e, index) {
    ...
    fetch(
      "https://dictionaryapi.com/api/v3/references/ithesaurus/json/" +
        word +
        "?key=68dac210-bb56-4bfb-bc92-b86dfcbda9d6"
    )
      .then((res) => res.json())
      .then(
        (result) => {
          ...
          if (result[0].meta === undefined) {
            ...
            alert(
              "Cannot find matched result, try again with these keywords ? \n" +
                word
            );
          } else {
      ...
      <div>
          ...
          onSubmit={(event) => {
            {
              this.props.nowBook.replace(/\s+/g, "").length > 0
                ...
                : alert("Please select one of your wordbook."),
                event.preventDefault();
            }
          }}
          style={{
        ...
        >
          <input
            type="submit"
            className="sendCard"
            value={this.state.docSend ? "Word Added !" : "Save"}
          ></input>
        </form>
      </div>
    );
