...
class BookDetail extends React.Component {
        ...
        .then(() => {
          if (this.state.allCards.length > 0) {
            ...
            if (this.state.allCards.length % 8 === 0) {
              this.setState({
                page: Math.floor(this.state.allCards.length / 8),
              });
            } else {
              this.setState({
                page: Math.floor(this.state.allCards.length / 8) + 1,
              });
            }
  ...
  componentDidUpdate() {
    ...
    if (isContentChange) {
      ...
      let isNoWords =
        this.state.lastValue < 1 && this.state.allCards.length < 1;
    ...
    return (
      <div key={i}>
        ...
        >
          ...
          <div className="popWordDetailContent">
            {data.antonym.length > 0 ? antonym : "No antonym"}
          </div>
        </div>
      </div>
    );
  ...
  renderWords() {
    return <div className="wordCardBox">{this.state.cards}</div>;
  }
  ...
  renderPage() {
    ...
    return <div className="pageLeaf">{All}</div>;
  }
    ...
    return (
      <div className="bookDetailBody">
        ...
        {this.state.cards.length > 0 ? (
          <div className="wordCardBox">{this.state.cards}</div>
        ) : (
          <div>
            {typeof this.state.cards === "object" ? (
              <div className="noWordsBlock">
                <div className="noWordsTitle">
                  There's no words in this wordbook
                </div>
                <div className="noWordSubTitle">
                  Try adding some words ! Add words with more then 2 synonyms
                  and antonyms to unlock quiz function.
                </div>
                <div
                  className="noWordBtn"
                  onClick={() => {
        ...
        >
          ...
          <div
            ...
            onClick={(event) => {
              event.stopPropagation(),
                (window.location.href =
                  "https://kiwords-c058b.web.app/quiz?" +
                  bookID +
                  "&" +
                  bookName);
            }}
